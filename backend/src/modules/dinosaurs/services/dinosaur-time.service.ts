  ...
  public adjustDinosaurStats(dinosaur: Dinosaur): Dinosaur {
    ...
    if (timeElapsedInSeconds > 0) {
        if (dinosaur.isSleeping) {
            // Régénération de l'énergie pendant le sommeil (appliquer le multiplicateur si gain positif)
            let energyRecovery = timeElapsedInSeconds * ENERGY_RECOVERY_RATE_PER_SECOND * dinosaur.multipliers.earn_energy_multiplier;
            energyRecovery *= dinosaur.multipliers.earn_energy_multiplier; // Appliquer le multiplicateur de gain d'énergie
  
            dinosaur.energy = Math.min(dinosaur.max_energy * dinosaur.multipliers.max_energy_multiplier, dinosaur.energy + energyRecovery);
